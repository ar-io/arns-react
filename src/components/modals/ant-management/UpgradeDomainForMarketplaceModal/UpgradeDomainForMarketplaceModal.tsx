...
function UpgradeDomainForMarketplaceModal({
  processId,
  visible,
  setVisible,
  domain,
    ...
    try {
      ...
      if (failedUpgrades.length) {
        ...
        eventEmitter.emit('success', {
          ...
          name: `Domain successfully updated for marketplace!`,
        });
  ...
  return (
    <div className="modal-container items-center justify-center">
      <div className="flex h-fit max-h-[50rem] w-[35rem] justify-between flex-col rounded-lg bg-[#1B1B1D] shadow-one">
        <div className="flex flex-row justify-between border-b-[1px] border-dark-grey px-8 py-4">
          ...
          >
            {signingMessage.length ? (
              ...
              'Upgrade Domain for Marketplace'
            )}
          </h1>
          <button
            disabled={upgrading}
        ...
        {!upgrading ? (
          <>
            <div className="flex box-border h-full overflow-hidden w-full flex-col px-8 py-4 text-sm text-white">
              <div className="flex flex-col gap-4 mb-4 border-b-[1px] border-dark-grey pb-4">
                <div className="bg-warning/10 border border-warning/20 rounded-lg p-4">
                  <h3 className="text-warning font-semibold mb-2">
                    Marketplace Compatibility Required
                  </h3>
                  <p className="text-sm text-white/80">
                    Your domain uses an ANT version that is not compatible with
                    the marketplace. To list your domain for sale, you must
                    upgrade to ANT version {minimumANTVersionForMarketplace} or
                    higher.
                  </p>
                </div>
              </div>
                    ...
                    message={
                      <div className="flex flex-col">
                        ...
                        <span className="pt-2 text-primary">
                          After upgrading, you'll be able to list your domain on
                          the marketplace.
                        </span>
                        <span className="pt-2 text-primary">
                          View the code:{' '}
        ...
        <div>
          ...
          >
            {!accepted
              ...
              : upgrading
                ? 'Upgrading for marketplace compatibility...'
                : 'Upgrade Domain'}
          </button>
        </div>
      </div>
...
export default UpgradeDomainForMarketplaceModal;
